...
if [ $(grep -c "docker_entrypoint.sh" $mergedListFile) -eq '0' ]; then
    ...
    echo -e >>$mergedListFile
    echo "52 */1 * * * docker_entrypoint.sh >> /AutoTaskScript/src/logs/default_task.log 2>&1" >> $mergedListFile
else
    echo "已配置默认脚本更新任务。"
fi
...
#echo "第5步判断是否配置自定义 shell 执行脚本..."
#if [ 0"$CUSTOM_SHELL_FILE" = "0" ]; then
#    echo "未配置自定shell脚本文件，跳过执行。"
#else
#    if expr "$CUSTOM_SHELL_FILE" : 'http.*' &>/dev/null; then
#        echo "自定义shell脚本为远程脚本，开始下载自定义远程脚本。"
#        wget -O /AutoTaskScript/docker/shell_script_mod.sh $CUSTOM_SHELL_FILE
#        echo "下载完成，开始执行..."
#        echo "#远程自定义shell脚本追加定时任务" >>$mergedListFile
#        sh -x /AutoTaskScript/docker/shell_script_mod.sh
...
#sh -x /AutoTaskScript/docker/proc_file.sh
